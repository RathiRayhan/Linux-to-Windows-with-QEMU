...
if [ $link1_status = "200" ] ; then 
	sudo wget -P /mediabots https://drive.darkmelodybd.workers.dev/0:/iso/WIN10.PRO.2004.SUPERLITE+COMPACT.U9.X64.GHOSTSPECTRE.(N-V3).iso http://download.microsoft.com/download/6/2/A/62A76ABB-9990-4EFC-A4FE-C7D698DAEB96/9600.17050.WINBLUE_REFRESH.140317-1640_X64FRE_SERVER_EVAL_EN-US-IR3_SSS_X64FREE_EN-US_DV9.ISO 
elif [ $link2_status = "200" -o $link2_status = "301" -o $link2_status = "302" ] ; then 
	sudo wget -P /mediabots https://drive.darkmelodybd.workers.dev/0:/iso/WIN10.PRO.2004.SUPERLITE+COMPACT.U9.X64.GHOSTSPECTRE.(N-V3).iso
else
